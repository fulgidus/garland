...
So should I be a classicist or a mockist?
Final Thoughts
I first came across the term “mock object” a few years ago in theExtreme Programming(XP) community. Since then I've run into mock objects more and more.
...
This difference is actually two separate differences. On the
		...
		classical and mockist styles ofTest Driven Development.
Regular Tests
...
xUnit tests follow a typical four phase sequence: setup, exercise,
			...
			method (setting up the order). The call toorder.fillis the
...
During setup there are two kinds of object that we are
			...
			forOrder.fillto work we also need an instance of
...
reasons: one is to get the tested behavior to work at all (sinceOrder.fillcalls warehouse's methods) and secondly I need
...
“secondary objects”)
This style of testing usesstate verification:
...
Concentrate ontestFillingRemovesInventoryIfInStockfirst, as I've taken
...
The key difference here is how we verify that the order did
			...
			state. Mocks usebehavior verification, where we instead check
...
create the mock differently, using  themockmethod
...
relaxed the constraints on the expectation by usingwithAnyArguments. The reason for this is that the first
...
the effort of migrating the tests. As it turns out I could have leftwithAnyArgumentsout entirely, as that is the default.
Using EasyMock
...
The vocabulary for talking about this soon gets messy - all
			...
			words to use.
Meszaros uses the termTest Doubleas the generic term for
...
Dummyobjects are passed around but never actually
		used. Usually they are just used to fill parameter lists.
Fakeobjects actually have working implementations, but
		usually take some shortcut which makes them not suitable for
		production (anin memory databaseis a good example).
Stubsprovide canned answers to calls made during the test,
		usually not responding at all to anything outside what's
		programmed in for the test.
Spiesare stubs that also record some information based
                on how they were called. One form of this might be an email
                service that records how many messages it was sent.
Mocksare what we are talking about here: objects
...
result the stub implementsMailServicebut adds extra
test methods.
...
Now I'm at the point where I can explore the second
			...
			here iswhento use a mock (or other double).
Theclassical TDDstyle is to use real objects if
			...
			much.
Amockist TDDpractitioner, however, will always
...
creating doubles.
An important offshoot of the mockist style is that ofBehavior Driven Development(BDD). BDD
...
With this style you begin developing auser storyby writing your first
...
theseObject Mothers, based on a naming convention used on an early
...
wrecks' - method chains of style ofgetThis().getThat().getTheOther(). Avoiding method chains
...
One of the hardest things for people to understand in OO
			...
			principle, which encourages you to tell an object to do
...
Mockists favorrole interfacesand assert that using
				...
				that domain, whichmaybe implemented by a string buffer.
...
series). He also maintains aweb
sitewith the patterns from the book.
To find out more about TDD, the first place to look isKent's
			book.
To find out more about the mockist style of testing, the
      best overall resource isFreeman & Pryce. The
      authors look aftermockobjects.com. In particular read theexcellent OOPSLA
paper. For more on Behavior Driven Development, a different
offshoot of TDD that is very mockist in style, start with Daniel Terhorst-North'sintroduction.
You can also find out more about these
techniques by looking at the tool websites forjMock,nMock,EasyMock, and the.NET EasyMock.
(There are other mock tools out there, don't consider this list to be
complete.)
XP2000 saw theoriginal mock
objects paper, but it's rather outdated now.
